window.onload = () => {
  ...
  // 🔥 PERSONAL RESULT PICK KARO (NOT MEDICAL)
  const data = JSON.parse(localStorage.getItem("personal_result"));

  if (!data) {
    alert("No personal loan result found");
  ...
  // 🔥 LOW SCORE / NO SCHEME (PERSONAL)
  if (!schemes || schemes.length === 0) {
    section.innerHTML += `
      <div class="no-loans">
        <h3>No personal loans available for this score</h3>
        <p style="opacity:0.7">
          Increase income, reduce EMIs or improve credit score to unlock options.
        </p>
      </div>
    `;
  } else {
...
if (boostBtn) {
  boostBtn.onclick = () => {
    ...
    if (!data) {
      alert("No personal loan data found");
      return;
    }

    // 🚀 redirect to boost page
    window.location.href = "boost_personal.html";
  };
}
};
