window.onload = () => {
  ...
  // 🔥 BUSINESS RESULT PICK KARO (NOT MEDICAL / PERSONAL)
  const data = JSON.parse(localStorage.getItem("business_result"));

  if (!data) {
    alert("No business loan result found");
  ...
  // 🔥 LOW SCORE / NO SCHEME (BUSINESS)
  if (!schemes || schemes.length === 0) {
    section.innerHTML += `
      <div class="no-loans">
        <h3>No business loans available for this score</h3>
        <p style="opacity:0.7">
          Improve cash flow, reduce liabilities or strengthen credit profile to unlock options.
        </p>
      </div>
    `;
  } else {
...
if (boostBtn) {
  boostBtn.onclick = () => {
    ...
    if (!data) {
      alert("No business loan data found");
      return;
    }

    // 🚀 redirect to boost page
    window.location.href = "boost_business.html";
  };
}
};
