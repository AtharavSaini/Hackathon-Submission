window.onload = () => {
  ...
  const summary = data.insights.summary || "Business loan profile";
  ...
  if (insightsData.length === 0) {
    container.innerHTML = `
      <div class="insight-card info">
        <h3>No major business risks detected</h3>
        <p class="impact">
          Your business profile looks strong for loan approval.
        </p>
        <p class="solution">
          Maintain healthy cash flow and credit discipline.
        </p>
      </div>
    `;
  } else {
  ...
  document.getElementById("exploreLoansBtn").onclick = () => {
    window.location.href = "result_business.html";
  };
};
